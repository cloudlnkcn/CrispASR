...
// (#205 / 617cd02 — see the chunk-context trim below for the history).
static std::string crispasr_rebuild_text_from_words(const std::vector<crispasr_word>& words) {
    std::string rebuilt;
...
// Issue #89 gap-fill: bounded-window backends (parakeet-ja) sometimes emit
// nothing for a multi-second span *inside* a slice — the encoder blanks an
// utterance whenever enough context follows it, even though the same span
// transcribes verbatim in isolation (measured: the reporter's 60 s clip's
...
// gap, and merge them back. Silence gaps re-transcribe to nothing and cost
// little. Gated by backend.vad_slice_cap_seconds() > 0 at the call site;
// CRISPASR_GAP_FILL=0 disables, CRISPASR_GAP_FILL_MIN_CS tunes the trigger.
static void crispasr_gap_fill_slice(CrispasrBackend& be, const whisper_params& params, const float* samples,
