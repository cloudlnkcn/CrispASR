...
//
// CrispASR TTS output is watermarked with a spread-spectrum frequency-
// domain pattern that survives re-encoding and moderate compression.
// crispasr_watermark_detect() returns a confidence score in [0, 1]:
//   > 0.65 — watermark present (AI-generated by CrispASR)
//   < 0.40 — no watermark detected
//
// Input: float32 mono PCM at any sample rate.
CRISPASR_API float crispasr_watermark_detect(const float* pcm, int n_samples);

// Embed the CrispASR watermark into float32 mono PCM (in-place).
// `alpha` controls strength (0.005 default, ~-46 dB — inaudible).
// Called automatically by TTS pipelines; exposed here for tools that
// generate audio outside the session API.
