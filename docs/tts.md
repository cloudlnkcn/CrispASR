...
| `QWEN3_TTS_CODEC_GPU` | auto | Force codec weights and decode through the GPU scheduler. The normal default already uses GPU on non-Metal GPU backends (CUDA/Vulkan/SYCL/CANN) and keeps Metal/CPU on the CPU codec path because the Metal transposed-conv chain can hang. Distinct from `QWEN3_TTS_CODEC_FORCE_METAL`, which also enables a per-op trace callback for debugging. |
| `QWEN3_TTS_CODEC_CPU` | unset | Force codec weights and decode through the CPU-only `codec_sched`, even when a non-Metal GPU backend is available. Useful for A/B timing and regression bisection. |
