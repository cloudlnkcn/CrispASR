...
|---|---|---|---|
| **MEDIUM** | [#52 Qwen3-TTS](#52-qwen3-tts) — perf pass | Medium | talker + code_predictor + codec + ECAPA + codec_encoder all done; step-4 perf pass open (~137 ms/frame → real-time). **O15 broken on CUDA and default-OFF** (`61c42bfb`) — main perf lever disabled; needs scheduler fix or alternative (FUSED_QKV, CP graph fusion). **2026-06-13 Kaggle P100 retest:** O15=OFF baseline works (27.4 ms/frame, WAV OK, ASR roundtrip passes); O15=ON still crashes (rc=-6 SIGABRT at 6.1s). Confirmed: not a stale-ggml issue — real scheduler bug. |
...
| **MOSTLY DONE** | [#58 MOSS-Audio-4B-Instruct](#58-moss-audio-4b-instruct) | Large | Runtime working, diff-validated cos ≥ 0.999, GGUFs published. Polish (tests, docs, HISTORY) shipped 2026-06-12. **2026-06-13 Kaggle P100:** first GPU run hit UTF-8 stderr decode bug in test harness (fixed `aa60ba99`); sweep v2 re-running. Remaining: GPU validation result, flash-attn for encoder. |
...
- **#72 Linux/CUDA validation** of the gemma4_e2b / mimo_asr GPU-residency flip. **2026-06-13 Kaggle P100 result: gemma4-e2b GPU=6.8s vs CPU=10.8s → 1.59× speedup** (JFK 11s Q4_K). Confirms the Metal wins translate to CUDA. mimo-asr GPU validation blocked by tokenizer download bug in sweep v1 (fixed `aa60ba99`, re-running).
