...
**Second half — still open.** `canary_transcribe_streamed` (parakeet-shape: global-z-norm + chunked-encode + concat, plus AED prompt re-injection at chunk boundaries — NeMo handles this in `FrameBatchMultiTaskAED`). Lift is ~1 day if the encoder is wired the same way as parakeet's. Less urgent now that we know the JA failure was the whitelist gap, not a long-audio bug — canary-1b-v2 only handles 4 short-form languages anyway.
