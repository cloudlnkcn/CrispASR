...
| **tada** | [`cstr/tada-tts-1b-GGUF`](https://huggingface.co/cstr/tada-tts-1b-GGUF) + `HumeAI/tada-3b-ml` | Llama-3.2 1B/3B backbone + per-token FM diffusion head + TADA codec at 24 kHz; 1:1 text-to-acoustic alignment; voice cloning via `--voice <ref.wav>` + `--codec-model <codec.gguf>` ([more](docs/architecture.md#tada)) | en | — |
