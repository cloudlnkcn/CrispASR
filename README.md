...
- `--diarize-method pyannote` — native GGUF pipeline (no Python, no sherpa-onnx). Pass `--sherpa-segment-model pyannote-v3-seg.gguf` for the pyannote v3 segmentation model. Falls back to sherpa subprocess for `.onnx` models. Pyannote runs once globally over the full audio for cross-slice consistency, ASR segments are split at speaker-turn boundaries (when per-word timestamps are available), and overlap classes contribute to both involved speakers. For globally stable speaker IDs anchored to physical speakers, add `--diarize-embedder auto` (issue [#107](https://github.com/CrispStrobe/CrispASR/issues/107)).
- `--diarize-embedder MODEL` — optional pluggable speaker-embedding model for the pyannote path. Each finalized segment gets a speaker embedding extracted, then agglomerative cosine clustering produces globally stable speaker IDs across the entire file (e.g. spk0 = same physical voice in every segment). `auto` downloads TitaNet-Large; pass a `.gguf` path to use a specific model. Tune with `--diarize-cluster-threshold X` (default 0.5, higher = more clusters) and `--diarize-max-speakers N` (default 8). Adapters for other embedders (ECAPA-TDNN, x-vector, …) drop in through the same factory. Without this flag, pyannote local-track IDs are used directly — fine for short single-pass audio, less reliable for long multi-speaker files.
- `--diarize-method sherpa` / `ecapa` — calls an externally-installed [sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx) subprocess with speaker embedding models. Use when you already have a sherpa-onnx workflow; otherwise the native `--diarize-method pyannote --diarize-embedder auto` combo is preferable (zero external deps, same kind of pipeline).
