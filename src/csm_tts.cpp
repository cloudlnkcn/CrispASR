...
extern "C" int csm_tts_run_backbone_dump(struct csm_tts_context* ctx, const char* text, int max_T,
                                         float* layer0_normed, float** layer_outputs, int n_layers, float* last_h,
                                         float* c0_logits) {
    if (!ctx || !text) {
        return -1;
    }
    ...
    for (uint32_t il = 0; il < hp.bb_n_layers; il++) {
        ...
        ggml_tensor* attn =
            core_attn::kv_self_attn(ctx0, gf, x, blk.attn_q_w, blk.attn_k_w, blk.attn_v_w, blk.attn_output_w, nullptr,
                                    nullptr, in_pos, (T == 1) ? nullptr : causal_mask, ctx->bb_kv_k, ctx->bb_kv_v,
                                    (int)il, 0, kvp);
        cur = ggml_add(ctx0, residual, attn);
...
extern "C" int csm_tts_run_depth_dump(struct csm_tts_context* ctx, const float* last_h, int c0_token,
    ...
    ggml_backend_tensor_set(ggml_graph_get_tensor(gf, "dd_positions"), dd_pos.data(), 0, dd_pos.size() * sizeof(int32_t));
    ggml_backend_tensor_set(ggml_graph_get_tensor(gf, "dd_mask"), dd_mask.data(), 0, dd_mask.size() * sizeof(ggml_fp16_t));
    if (ggml_backend_sched_graph_compute(ctx->sched, gf) != GGML_STATUS_SUCCESS) {
        ggml_free(ctx0);
        return -4;
    }
